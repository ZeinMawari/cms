...
class Konten extends CI_Controller {
    ...
    public function simpan(){
        ...
        $config['max_size']         = 900 * 1024; //3 * 1024 * 1024; //3m; 0=unlimited
        $config['file_name']        = $namafoto;
        $config['allowed_types']    = '*';
        $this->load->library('upload',$config);
        if($_FILES['foto']['size'] >= 900 * 1024){
    ...
    public function update(){
        ...
        $config['upload_path']      = 'assets/upload/konten';
        $config['max_size']         = 500 * 1024; //3 * 1024 * 1024; //3m; 0=unlimited
        $config['file_name']        = $namafoto;
        ...
        public function cari() {
            ...
            // Model untuk pencarian produk
            $this->load->model('Produk_model');
            
            // Menampilkan produk yang sesuai dengan pencarian
            $data['produk'] = $this->Produk_model->search_products($keyword);
            
            // Mengirimkan data ke view
            $this->load->view('produk/cari', $data);
        }
    }
